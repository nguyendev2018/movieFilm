...
class AccountController extends Base
{
    ...
    public function index(): void
    {
        ...
        $data["accounts"] = $account_model->getAllAccounts($limit, $offset);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if ($keyword) {
                ...
                $data["accounts"] = empty($result) ? array() : (isset($result[0]) ? $result : array($result));
            }
        }
...

{
    ...
    if ($user_id) {
        $data["account"] = $account_model->findAccountById($user_id);
    } else {
        $data["account"] = [
            "username" => "",
            "role" => "",
